...
// ── StatusButton — כפתור ורוד עם תפריט נפתח ─────────────────────────────
function StatusButton({ task, updateTaskStatus }) {
  ...
  const handleSelect = async (newStatus) => {
    ...
    try {
      ...
      console.error('[StatusButton] update error:', e);
    } finally {
      setUpdating(false);
    }
  };
  ...
  return (
    <div className="relative shrink-0">
      ...
      {open && (
        <>
          {/* Backdrop — סגירה בלחיצה מחוץ לתפריט */}
          <div
            className="fixed inset-0 z-10"
            onClick={() => setOpen(false)}
          />
          <div
            className="absolute left-0 top-full mt-1 z-20 min-w-[110px]
...
function TasksWidget({ onNavigateToTasks }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200
                    dark:border-gray-700 shadow-sm p-5 h-full transition-colors">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          {openCount > 0 && (
            <span className="bg-[#e5007e] text-white text-xs font-bold px-2 py-0.5 rounded-full">
              {openCount}
            </span>
          )}
        </div>
        <button
          ...
          className="flex items-center gap-1 text-xs text-[#e5007e] font-semibold hover:underline"
        >
          כל המשימות
      ...
      /* Empty */
        ...
        <>
          ...
          {/* Urgent tasks — with status button */}
...
export default function Dashboard({ currentPage, setCurrentPage }) {
  ...
  const thisMonthIncomes  = incomes.filter(i  => { const d = new Date(i.date);  return d.getMonth() === currentMonth && d.getFullYear() === currentYear; });
  const thisMonthExpenses = expenses.filter(i => { const d = new Date(i.date);  return d.getMonth() === currentMonth && d.getFullYear() === currentYear; });
  ...
  const barData = Array.from({ length: 6 }, (_, i) => {
    ...
    const inc = incomes.filter(x  => { const d = new Date(x.date); return d.getMonth() === m && d.getFullYear() === y; }).reduce((s, x) => s + x.amount, 0);
    const exp = expenses.filter(x => { const d = new Date(x.date); return d.getMonth() === m && d.getFullYear() === y; }).reduce((s, x) => s + x.amount, 0);
  ...
  const prevMonthIncome  = incomes.filter(i  => { const d = new Date(i.date);  return d.getMonth() === prevMonth && d.getFullYear() === prevMonthYear; }).reduce((s, i) => s + i.amount, 0);
  const prevMonthExpense = expenses.filter(i => { const d = new Date(i.date);  return d.getMonth() === prevMonth && d.getFullYear() === prevMonthYear; }).reduce((s, i) => s + i.amount, 0);

  const incomeChange  = prevMonthIncome  > 0 ? ((totalIncome   - prevMonthIncome)  / prevMonthIncome  * 100).toFixed(1) : null;
  const expenseChange = prevMonthExpense > 0 ? ((totalExpenses - prevMonthExpense) / prevMonthExpense * 100).toFixed(1) : null;
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            {/* הכנסות */}
            <div className="gsap-card bg-white dark:bg-gray-800 rounded-xl p-4 md:p-5 shadow-sm border border-gray-200 dark:border-gray-700 transition-colors h-full">
