    ...
    <div className="gsap-card bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden transition-colors">
      <div className="p-4 md:p-5 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
        ...
        <button onClick={onExpand}
          className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors" title="הרחב">
...
export default function Reports() {
  ...
  const filteredMonthlyData = useMemo(() => {
    ...
    if (monthlyFilter.showOnlyActive) {
      data = data.filter(m => m.income > 0 || m.expenses > 0);
    }
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      <LineChart data={monthlyData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                        ...
                        <Line type="monotone" dataKey="income" stroke="#10b981" strokeWidth={2.5}
                          name="הכנסות" dot={{ r: 4, fill: '#10b981' }} activeDot={{ r: 6 }} />
                        <Line type="monotone" dataKey="expenses" stroke="#e5007e" strokeWidth={2.5}
                          name="הוצאות" dot={{ r: 4, fill: '#e5007e' }} activeDot={{ r: 6 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                ...
                <div className="gsap-card bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden transition-colors">
                  <div className="p-3 md:p-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
                    ...
                    <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                      <select
                        value={monthlyFilter.sortBy}
                        onChange={(e) => setMonthlyFilter(prev => ({ ...prev, sortBy: e.target.value }))}
                      ...
                      <button
                        onClick={() => setMonthlyFilter(prev => ({ ...prev, showOnlyActive: !prev.showOnlyActive }))}
                        ...
                        <div key={idx} className={`p-4 ${!hasActivity ? 'opacity-40' : ''}`}>
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className={`text-sm font-bold px-2 py-0.5 rounded-full ${
                              profit >= 0 ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400'
                            }`}>
                                ...
                                {hasActivity ? (
                                  profit >= 0 ? (
                                    ...
                                    <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-full text-xs font-medium">
                                      <TrendingDown className="w-3 h-3" />הפסד
                                    </span>
                                  )
                                ) : (
