...
const DEFAULT_CATEGORIES = [
  { name: 'מוצרים', color: '#e5007e' },
  { name: 'הוצאות קבועות', color: '#ef4444' },
  { name: 'חשבונות', color: '#f97316' },
  { name: 'שכר עובדים', color: '#8b5cf6' },
  { name: 'שיווק', color: '#3b82f6' },
...
export default function Expenses() {
  ...
  const stats = useMemo(() => {
    ...
    const thisMonthTotal = thisMonthExpenses.reduce((s, i) => s + i.amount, 0);
    const prevMonthTotal = prevMonthExpenses.reduce((s, i) => s + i.amount, 0);
    ...
    const maxExpense = expenses.reduce((max, i) => i.amount > max.amount ? i : max, expenses[0]);
  ...
  const totalFiltered = useMemo(() =>
    filteredExpenses.reduce((sum, i) => sum + i.amount, 0),
  [filteredExpenses]);
  ...
  const handleExportCSV = useCallback(() => {
    ...
    link.href = url;
    link.download = `הוצאות_${new Date().toLocaleDateString('he-IL').replace(/\//g, '-')}.csv`;
    link.click();
